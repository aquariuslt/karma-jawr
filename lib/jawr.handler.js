...
var constructFlattenJawrCssBundles = function(jawrContext) {
  ...
  var calculateNonCompositeChildKeys = function() {
    var calculateNonCompositeChildKeys = function(bundleKey, bundleKeyCurrentMappings, bundleMappings) {
      if (bundleMappings[bundleKey] && bundleMappings[bundleKey].composite === true) {
        var childKeys = helpers.splitAndTrim(bundleMappings[bundleKey].child.names);
        _.each(childKeys, function(childKey) {
...
var generateFlattenMappingHashFlag = function(jawrContext) {
  ...
  var generateCurrentFlattenMappingFlag = function(targetFilePath, jsBundleFlattenMappings) {
    ...
    var formattedJsBundleFlattenMappings = {};
    _.each(_.keys(jsBundleFlattenMappings), function(flattenBundleKey) {
      formattedJsBundleFlattenMappings[flattenBundleKey] = jsBundleFlattenMappings[flattenBundleKey];
      formattedJsBundleFlattenMappings[flattenBundleKey].mappings = _.map(jsBundleFlattenMappings[flattenBundleKey].mappings, function(mappingString) {
        return mappingString.replace(new RegExp(path.sep, 'g'), '/');
      });
    });
    fs.writeFileSync(targetFilePath, JSON.stringify(formattedJsBundleFlattenMappings));
  };
