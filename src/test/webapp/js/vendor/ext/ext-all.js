...
//                    if (h.id.indexOf('tab') > -1 || h.id == 'fdoc.blc.party.panel.notifyParties' || h.id == 'fdoc.blc.form.blCreation' || h.id == 'fdoc.blc.panel.memoPanel') {
//                        console.log('slow dolayout for ' + (end - start) + ' ms: ' + h.id);
//                    }
				}
			}
		}
			...
			onRender : function(b, a) {
				...
				if (this.boxLabel) {
					this.wrap.createChild({
								tag : "label",
								htmlFor : this.el.id,
								cls : "x-form-cb-label "+this.cls, //add cls to Label tag in checkbox and radiobox
								html : this.boxLabel
							})
				}
				if (this.checked) {
